...
function load_list_data() {
    ...
    sessionStorage.removeItem("incEdit");
    /*
            ...
            oSettings.jqXHR = $.ajax({
                ...
                "success": function (result) {
                    ...
                    for (var key in result["incidents"]) {
                        ...
                        var t =
                        objinner.push(UtcToLocal(getValue(result["incidents"][key]['date']) + " " + getValue(result["incidents"][key]['time']), 'M/D/YY hh:mm:ss A'));
    ...
    */
    ...
    Search(obj, 'ClaimDeskWeb/services/v1/search/incident/advancedSearch');
...
function incident_step_one() {
    if ($("#incident_one_frm").valid()) {
        ...
        location.href = 'create-incident.html';
    }
}
// function to load incident step two page with incident info etc.
function incident_step_two() {
    if ($("#incident_two_frm").valid()) {
        ...
        location.href = 'create-incident1.html';
    }

}
// function to load incident step three page with witness info etc.
function incident_step_three() {    
    if ($("#incident_three_frm").valid()) {
        var holder, li, clone, counter, divcnt;

        holder = $("#container");
        divs = holder.find('.claimForm');
        divcnt = divs.length;
        var witness = [];
        var filename = [];
        var c = [];
...
function incident_step_four() {
    ...
    var UtcincDtTime = localToUtc(inc_date + " " + (inc_time != 'NA' ? inc_time : ''), 'MM/DD/YYYY hh:mm A');
    var UtcreportedDtTime = localToUtc(inc_date_report + " " + (inc_time_reported != 'NA' ? inc_time_reported : ''), 'MM/DD/YYYY hh:mm A');
    ...
    var jsonData = {
        ...
        "country": inc_county,
        "incidentDate": getDate(UtcincDtTime, 'MM/DD/YYYY'),
        "incidentTime": getTime(UtcincDtTime, 'hh:mm A'),
        "reportedDate": getDate(UtcreportedDtTime, 'MM/DD/YYYY'),
        "reportedTime": getTime(UtcreportedDtTime, 'hh:mm A'),
        "fax": inc_fax,
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            sessionStorage.setItem("incident_id", result);
            removeSessionvar_onsave();
            removeSessionvar();
            location.href = "create-incident3.html";
        }
    });
...
function callfunc(flag) {
    ...
    setlocId(sessionStorage.getItem('incident_id'), flag);
}

function setclaimlocId(id, claimType) {
    debugger;
    if (claimType == 'GL') {
        ...
        //var lid = id;
        sessionStorage.setItem("claimEditWC", id);
        sessionStorage.setItem("claimtype", "WC");
        sessionStorage.setItem("incLocId", gIncLodId);
        sessionStorage.setItem("incident_id", gIncId);
...
function setlocId(locId, flag) {
    ...
    if (flag == 'show') {
        ...
        $.ajax({
            ...
            success: function (result) {
                sessionStorage.setItem('incident_id', result["incidentId"])
                sessionStorage.setItem("img_incId", result["incidentId"]);
                ...
                $("#inc_date").html(UtcToLocal(checkdate(result['incidentDate']) + " " + getValue(result['incidentTime']), 'M/D/YYYY hh:mm A'));
                $("#date_report").html(UtcToLocal(checkblank(result['reportedDate']) + " " + getValue(result['reportedTime']), 'M/D/YYYY hh:mm A'));
                $("#desc").html(checkblank(result['description']));
                ...
                var str1 = '<li class="popInn"><h2 class="popSubHead">Attachments<input  id="btn_add_inc_attatchment" class="popBtt buttonNex" type="button" onclick="btn_add_inc_attatchment(' + locId + ')" name="" value="Add Attachment" /></h2>';
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    cache: false,
                    url: hostname + "ClaimDeskWeb/services/v1/search/getRelatedClaimList/" + locId,
                    headers: {
                        "token": token,
                        "userid": userId,
                        "locationId": locationId
                    ...
                    success: function (result1) {                        
                        gIncLodId = $('#li_' + locId).attr('data-inc-loc');
                        sessionStorage.setItem("incLocId", gIncLodId);
                        var sttr;
                        if (result['status'] == "APPROVED" || result['status'] == "DECLINED") {
                            sttr = '<li class="popInn"><h2 class="popSubHead">Related Claim List<input  id="close_but" class="popBtt buttonNex" type="button" aria-label="Close" onClick="msg1(' + '\'' + result['status'] + '\'' + ')" name="" value="Create Claim"></h2><br><ul class="subSection" ><li style="width:5%;background-color:#ebebeb;"><strong>Sr. No.</strong></li><li style="width:15%;background-color:#ebebeb;"><strong>Claim ID</strong></li><li style="width:20%;background-color:#ebebeb;"><strong>Claim Date</strong></li><li style="width:38%;background-color:#ebebeb;"><strong>Description</strong></li><li style="width:7%;background-color:#ebebeb;"><strong>Claim Type</strong></li><li style="width:5%;background-color:#ebebeb;">&nbsp;&nbsp;</li></ul>';
                        }
                        else {
                            sttr = '<li class="popInn"><h2 class="popSubHead">Related Claim List<input  id="close_but" class="popBtt buttonNex" type="button" aria-label="Close" onClick="openModal();" name="" value="Create Claim"></h2><br><ul class="subSection"><li style="width:5%;background-color:#ebebeb;"><strong>Sr. No.</strong></li><li style="width:15%;background-color:#ebebeb;"><strong>Claim ID</strong></li><li style="width:20%;background-color:#ebebeb;"><strong>Claim Date</strong></li><li style="width:38%;background-color:#ebebeb;"><strong>Description</strong></li><li style="width:7%;background-color:#ebebeb;"><strong>Claim Type</strong></li><li style="width:5%;background-color:#ebebeb;">&nbsp;&nbsp;</li></ul>';
                        }
                        var i = 1;
                        for (var key in result1) {                            
                            var desc = result1[key]['description'];
                            if (typeof desc == 'undefined' || desc == '' || desc == 'NA') {
                                desc = result1[key]['title'];
                                if (typeof desc == 'undefined' || desc == '' || desc == 'NA')
                                    desc = '&nbsp;&nbsp;';
                            }
                            ...
                            if (result['status'] == "APPROVED" || result['status'] == "DECLINED") {
                                sttr += '<ul class="subSection"><li style="width:5%;">[' + (i++) + ']</li><li style="width:15%; cursor:pointer;"><span onclick="msg2(' + result1[key]['status'] + ')"><b>' + result1[key]['claimId'] + '</b></span></li><li style="width:20%;">' + UtcToLocal(result1[key]['incidentDate'], 'MM/DD/YYYY hh:mm A') +
                                '</li><li style="width:38%;">' + desc + '</li><li style="width:7%;">' + result1[key]['claimType'] + '</li><li style="width:5%;"><img src="images/' + (result1[key]['type'] == 'GL' ? 'typeGL.gif' : 'typeWC.gif') + '"/></li></ul>';
                            }
                            else {
                                sttr += '<ul class="subSection"><li style="width:5%;">[' + (i++) + ']</li><li style="width:15%; cursor:pointer;"><span onclick="setclaimlocId(' + result1[key]['claimId'] + ', \'' + result1[key]['type'] + '\')"><b>' + result1[key]['claimId'] + '</b></span></li><li style="width:20%;">' + UtcToLocal(result1[key]['incidentDate'], 'MM/DD/YYYY hh:mm A') + '</li><li style="width:38%;">' + desc + '</li><li style="width:7%;">' + result1[key]['claimType'] + '</li><li style="width:5%;"><img src="images/' + (result1[key]['type'] == 'GL' ? 'typeGL.gif' : 'typeWC.gif') + '"/></li></ul>';
                            }
                        }
                        sttr += '<br><br></li>';
                        if (result1.length > 0)
        ...
        location.href = "edit-incident.html";
    }
    else if (flag == 'delete') {
            ...
            if (r) {                
                $.ajax({
                    ...
                    error: function (request, status, error) {
                        if (typeof request.responseJSON != 'undefined' && request.responseJSON != '') {
                            var msg = request.responseJSON['errors']['error'][0]['description'];
                            apprise(msg);
                        }
                        else if (request.status == '200' && request.statusText == 'OK') {
                            location.reload();
                        }
                    }
...
function createPageLoad() {
    var currentemp = sessionStorage.getItem("incEdit");
    ...
    if (currentemp) {
        $.ajax({
            ...
            success: function (result) {
                sessionStorage.setItem('incLocId', result.locationId)
                getIncidentLocation(result.locationId, result.incidentLocation != 'NA' ? result.incidentLocation : '', true);
                var is_add_inc_attatchment = sessionStorage.getItem('Ssn_is_Add_inc_Attatchment') == 'true' ? true : false;
                if (is_add_inc_attatchment) {
                    $('a[href="#4"]').tab('show');
                    sessionStorage.removeItem('Ssn_is_Add_inc_Attatchment');
                }

                var incDtTime = UtcToLocal(result['incidentDate'] + ' ' + result['incidentTime'], 'MM/DD/YYYY hh:mm A');
                var repDtTime = UtcToLocal(result['reportedDate'] + ' ' + result['reportedTime'], 'MM/DD/YYYY hh:mm A');
                ...
                $("#inc_date").val(getDate(incDtTime, 'MM/DD/YYYY'));
                $("#date_reported").val(getDate(repDtTime, 'MM/DD/YYYY'));
                $("#inc_time").val(getTime(incDtTime, 'hh:mm A'));
                $("#time_reported").val(getTime(repDtTime, 'hh:mm A'));
                ...
                if (result['witness'] && result['witness'].length > 0) {
                    ...
                    for (var t = 1, b = 0; b < result['witness'].length; t++, b++) {
                        ...
                        str += '<div class="form2"><label class="myLable">Witness First Name<span style="color:red;">&nbsp;*</span></label><input name="wit_name' + t + '" id="wit_name' + t + '" type="text" class="planeTextFild wit_name" tabindex="' + ext + 2 + '" required="required" value="' + (obj['firstName'] != null ? obj['firstName'] : '') + '"><label class="myLable">Address 2</label><input name="wit_address_two' + t + '" type="text" class="planeTextFild wit_address1" placeholder="" tabindex="' + ext + 5 + '"  value="' + (obj['address1'] != null ? obj['address1'] : '') + '"/><label class="myLable">Zip Code</label><input name="wit_zip' + t + '" type="text" class="planeTextFild wit_zip" placeholder=""  tabindex="' + ext + 8 + '" value="' + (obj['zipcode'] != null ? obj['zipcode'] : '') + '"><label class="myLable">Work/Other Phone Number</label><input name="wit_work_phone' + t + '" type="text" class="planeTextFild wit_work_phone" placeholder="" onKeyUp="FormatPhone(event,this)" tabindex="' + ext + 11 + '" value="' + (obj['workPhone'] != null ? obj['workPhone'] : '') + '"></div><div class="form2"><label class="myLable">Witness Last Name</label><input name="wit_lastname' + t + '" tabindex="' + ext + 3 + '" type="text" class="planeTextFild wit_lastname" placeholder=""  value="' + (obj['lastName'] != null ? obj['lastName'] : '') + '"><label class="myLable">City</label><input name="wit_city' + t + '" tabindex="' + ext + 6 + '" type="text" class="planeTextFild wit_city" placeholder=""  value="' + (obj['city'] != null ? obj['city'] : '') + '"><label class="myLable">Country</label><input name="wit_country' + t + '" type="text" class="planeTextFild wit_country" placeholder=""  tabindex="' + ext + 9 + '" value="United States"></div><div class="form2"><label class="myLable">Address1</label><input name="wit_address' + t + '" type="text" class="planeTextFild wit_address" placeholder=""  tabindex="' + ext + 4 + '"  value="' + (obj['address'] != null ? obj['address'] : '') + '"/><label class="myLable">State</label><select name="wit_state' + t + '" class="form-control myselectBig wit_state"  tabindex="' + ext + 7 + '">' + strstates + '</select><label class="myLable">Home/Cell Phone Number</label><input name="wit_home_phone' + t + '" type="text" class="planeTextFild wit_home_phone" placeholder="" onKeyUp="FormatPhone(event,this)" tabindex="' + ext + 10 + '" value="' + (obj['homePhone'] != null ? obj['homePhone'] : '') + '"></div><div class="clear"></div></div>';
                        wtstate[t] = obj['state'];
                    }
                }
...
function incident_up() {
    ...
    if ($("#data").valid()) {
        ...
        $.ajax({
            ...
            data: JSON.stringify(jsonData),
            success: function (result) {
                location.href = "incidents-list.html";
...
function load_data_last() {
    $("#fname").html("Incident ID : " + sessionStorage.getItem("incident_id"));
    get_files(sessionStorage.getItem("incident_id"));
}

function get_files(incident_id) {
    //alert(sessionStorage.getItem("incident_id"));
...
function download_IncidentFile(filename) {    
    var currentIncident = sessionStorage.getItem("incEdit");
    if (!currentIncident) {
