...
function SortByAreaId(a, b) {    
    var x = parseInt(a.location.storeId.split(' ')[1]);
    var y = parseInt(b.location.storeId.split(' ')[1]);
...
function YTDInjuryByArea(divisionId) {
    ...
    var YearArray = [moment().year() - 1, moment().year()];
    ...
    $.when(promise).done(function (result) {
        var data = new Array();
        if (typeof result[0].location != 'undefined' && result[0].location != null) {
            var _dataObj1 = {
                location: result[0].location.sort(SortByAreaId),
                year: result[0].year
            };

            var _dataObj2 = {
                location: result[1].location.sort(SortByAreaId),
                year: result[1].year
            };
...
function ChargeBackReport(divisionId) {
    ...
    var promise = apiGetCall('ClaimDeskWeb/services/v1/reportSearch/chargeBackReport/' + divisionId + '/' + moment().year());
    $.when(promise).done(function (data) {
        if (true) {//data.length > 0
...
function employeeInjuryByCompare(divisions, textArr, period) {    
    var YearArray = [moment().year() - 2 ,moment().year() - 1, moment().year()];
    var promises = [];
    ...
    $.when(all(promises)).then(function (data) {
        var categories = ['P' + period + '-' + YearArray[0] + '', 'P' + period + '-' + YearArray[1] + '', 'P' + period + '-' + YearArray[2] + ''];
