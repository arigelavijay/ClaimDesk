...
function dashboardload() {    
    ...
    $.ajax({
        ...
        dataType: "json",
        cache: false,
        url: hostname + "ClaimDeskWeb/services/v1/search/count",
        headers: {
            "token": token,
            "userid": userId,
    ...
    //							dataType: "json",
    //							cache: false,
    //							url: hostname+"ClaimDeskWeb/services/v1/search/getClaimList/0",     
    //							headers: {"token": token,
    //									 "userid": userId,
    //									 "locationId": locationId},        
    //							success:function(result){	
    //								populateDate(result,0);
    ...
    //										 title="ACTIVE";
    //									 }
    //									 else if(result[key]['status']=="10")
    //									 {
    // 										 image="approved.png";
    //										  title="APPROVED";
    //									 }
    //									 else if(result[key]['status']=="50")
    //									 {
    //										 image="declined.png";
    //										  title="DECLINED";
    //									 }
    ...
    //										if(result[key]['status']=="0")
    //										{
    //											str = "<ul class='manageIcon1'><li><img title='"+title+"' src='images/"+image+"' alt='img'></li><li data-toggle='modal' data-target='.bs-example-modal-lg'><img src='images/min1.png' width='16' height='19' alt='img' onclick='setlocId("+result[key]['claimId']+",\"show\")'></li><li><a href='#'><img src='images/min2.png' width='16' height='19' alt='img' onclick=\"setlocId('"+result[key]['claimId']+"','edit')\"></a></li></ul>";
    //										}
    //										else
    //										{
    //										str = "<ul class='manageIcon1'><li><img title='"+title+"' src='images/"+image+"' alt='img'></li><li data-toggle='modal' data-target='.bs-example-modal-lg'><img src='images/min1.png' width='16' height='19' alt='img' onclick='setlocId("+result[key]['claimId']+",\"show\")'></li><li><a href='#'><img src='images/min2.png' width='16' height='19' alt='img' onclick=\"msg("+result[key]['status']+")\"></a></li></ul>";	
    //										}
    //									 objinner.push(str);						
    //								     var datecreated =  result[key]['created'];
    //									 var dateupdated =  result[key]['updated'];
    //							if (datecreated != dateupdated && userId ==result[key]['userId'] ) dataSet2.push(objinner);
    //							else if (userId == result[key]['userId'] ) dataSet1.push(objinner);
    //							else if (datecreated != dateupdated && userId != result[key]['userId']) dataSet4.push(objinner);
    //							else if (datecreated == dateupdated && userId != result[key]['userId'] )   dataSet3.push(objinner);									 						
    //						  }									  
    //						   
    //						
    //							$('#claimTable0').dataTable( {					
    //							  "bStateSave": true,
    //							  "bPaginate": false,
    //							  "bFilter": false,					 						
    //							  "bProcessing": true,                    
...
function loadDashboardTable(option) {
    $.ajax({
        ...
        dataType: "json",
        cache: false,
        url: hostname + "ClaimDeskWeb/services/v1/search/getClaimList/" + option,
        headers: {
            "token": token,
            "userid": userId,
            "locationId": locationId
...
function populateDate(result, option) {
    ...
    for (var key in result) {        
        ...
        var objinner = [];        
        objinner.push(result[key]['claimId']);
        objinner.push(UtcToLocal(result[key]['incidentDate'], 'MM/DD/YYYY hh:mm A'));
        objinner.push(result[key]['guestName'])
        objinner.push(result[key]['claimType']);
        if (userId != result[key]['userId']) {
            objinner.push(getValue(result[key]['username']));
        }
        objinner.push(result[key]['branchId'] + ", " + result[key]['address']);
        var image;
        var title;
        ...
        objinner.push(UtcToLocal(result[key]['created'], 'MM/DD/YYYY hh:mm A'));
        var str;
        if (result[key]['status'] == "ACTIVE") {
            str = "<ul class='manageIcon1'><li><img  width='16' height='16'  title='" + title + "' src='images/" + image + "' alt='img'></li><li data-toggle='modal' data-target='" + (result[key].type == "WC" ? ".view_claim_modal_wc" : ".bs-example-modal-lg")
                + "'><img id='img_" + result[key]['claimId'] + "' data-claim-type='" + result[key].type + "' data-hasdiary-note='" + (result[key]['diaryNotes'] ? true : false) + "' data-incident-locid='" + result[key]['locationId'] + "' src='images/min1.png' width='16' height='19' alt='img' onclick='setlocId("
                + result[key]['claimId'] + ",\"" + (result[key].type == "WC" ? "show_WC" : "show") + "\")'></li><li><img src='images/min2.png' width='16' height='19' alt='img' onclick=\"setlocId('"
            ...
            str = "<ul class='manageIcon1'><li><img  width='16' height='16' title='" + title + "' src='images/" + image + "' alt='img'></li><li data-toggle='modal' data-target='" + (result[key].type == "WC" ? ".view_claim_modal_wc" : ".bs-example-modal-lg")
                + "'><img id='img_" + result[key]['claimId'] + "' data-claim-type='" + result[key].type + "' data-hasdiary-note='" + (result[key]['diaryNotes'] ? true : false) + "' data-incident-locid='" + result[key]['locationId']
                + "' src='images/min1.png' width='16' height='19' alt='img' onclick='setlocId(" + result[key]['claimId'] + ",\"" + (result[key].type == "WC" ? "show_WC" : "show") + "\")'></li><li><img src='images/min2.png' width='16' height='19' alt='img' onclick=\"msg("
                + result[key]['status'] + ")\"></li>" + DiaryNoteImage(result[key]['diaryNotes'], result[key]['claimId'])
...
function getLocationDetails(locationId) {
    $.ajax({
        ...
        dataType: "json",
        cache: false,
        url: hostname + "ClaimDeskWeb/services/v1/location/" + locationId + "?pref=true",
        headers: {
            "token": sessionStorage.getItem("token"),
            "userid": sessionStorage.getItem("userid"),
            "locationId": sessionStorage.getItem("locationId")
        ...
        success: function (result) {
            ...
            if (result.config) {
                sessionStorage.setItem('ssnConfig', JSON.stringify(result.config));
            }
...
function btn_Create_OnClick(claim_id) {
    ...
    if (claim_id != -1) {
        ...
        if (btnObj.length > 0) {
            sessionStorage.setItem('incLocId', $('#img_' + claim_id)[0].getAttribute('data-incident-locid'));
    ...
    if (flag == 'show') {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                var str1 = '<li class="popInn"><h2 class="popSubHead">Attachments<input  id="btn_add_claim_attatchment" class="popBtt buttonNex" type="button" onclick="btn_add_claim_attatchment(' + locId + ',\'GL\')" name="" value="Add Attachment" /></h2>';
    ...
    if (inc_id && inc_id != '') {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $("#inc_date").html(incDtTime);
                $("#date_report").html(repDtTime);
