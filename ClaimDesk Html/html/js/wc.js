...
function dashboardloadOnInit() {
    ...
    var url = "../jsp/report.jsp?location=" + sessionStorage.getItem('incLocId') + "&Type=WC" + '&WebDir=html';
    //test
    /*
...
function initialiseComponents() {
    ...
    $('.hasDatepicker').mask("99/99/9999", { placeholder: "_" });
    ...
    if (ssnConfig != null && typeof ssnConfig.SHOW_INCIDENT_LOCATION != 'undefined' && ssnConfig.SHOW_INCIDENT_LOCATION == 'true') {
        ...
        set_codes_list_occupation('E_OCCUPATION', '#occupation', '', sessionStorage.getItem('incLocId'));
    }
    else {
    ...
    $('.empCss').on('change', function () {
        if (this.value != null && this.value != '') {
            ...
            $.when(promise).done(function (data) {
                /* replace occupation value with occupationcode value start */
                data.occupation = data.occupationCode;
                /* replace occupation value with occupationcode value end */
    ...
    $('[required="required"]').prev().append('<span style="color:red;">&nbsp;*</span>');
    
    $.validator.addMethod('ssnCss', function (value, element) {
        return value.replace(/\D/g, '').length == 9 ? true : false;
    }, 'SSN no. should contain 9 digits');

    $.validator.addMethod('dtPCss', function (value, element) {
        if (value == null || value == '')
            return true;
        return moment(value, 'MM/DD/YYYY').isValid() ? true : false;
    }, 'Not a valid date');
    
    $.validator.addMethod('zipCss', function (value, element) {
        return value.replace(/\D/g, '').length == 9 ? true : false;
    }, 'Zipcode should contain 9 digits');

    $.validator.addMethod('PhoneCss', function (value, element) {
    ...
    $.each($('.frmCss'), function (i, item) {        
        $('#' + item.id).validate().form();
    });
...
function set_codes_list_occupation(code, fieldname, preval, locId) {
    $.ajax({
        type: "GET",
        contentType: "application/json;charset=utf-8",
        accept: "application/json",
...
function BindDataToControls(tabId, i, data) {
    ...
    $(elementArr).each(function () {
        ...
        if (type == 'checkbox' || type == 'radio') {
            ...
            /* for binding text value of select tag of occupation or value if value field start */
            if (this.id == 'occupation') {
                var occupatioFlag = false;
                var selectedVal = data[this.id];

                $("#occupation option").filter(function () {
                ...
                if(!occupatioFlag)
                    $('#' + this.id + ' option[value=' + selectedVal + ']').prop('selected', 'selected');

                /* for binding text value of select tag of occupation or value if value field end */
            }
            else {
