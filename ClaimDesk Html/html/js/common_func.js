...
function toggle_visibility(val, objid)
{
    ...
    if (objid == 'file_upload_div') {
        ...
        if (val == true || val == 'Yes') {
            $('#d_location').addClass('checkblankC');
            $('#fault_pro').addClass('checkblankC');
        }
        else
        {
            $('#d_location').removeClass('checkblankC');
            $('#fault_pro').removeClass('checkblankC');
...
function getLocAdd(locid) {
    ...
    $.ajax({
        ...
        success: function (result) {            
        	sessionStorage.setItem("incLocId", result['id']);
...
function btn_add_inc_attatchment(incident_id, type) {
    ...
    sessionStorage.setItem('incEdit', incident_id);
    location.href = 'edit-incident.html';
    sessionStorage.setItem('Ssn_is_Add_inc_Attatchment', true);
}

function btn_add_claim_attatchment(claim_id, type) {
    
    sessionStorage.setItem('claimEdit', claim_id);    
    sessionStorage.setItem('Ssn_is_Add_claim_Attatchment', true);

    if (type == 'WC') {
        var incLocId = $('#img_' + claim_id)[0].getAttribute('data-incident-locid');
        sessionStorage.setItem('incLocId', incLocId);
...
function getJspPageContent(incidentLocId) {
	var def = $.Deferred();	
    var url = '../jsp/report.jsp?location=' + incidentLocId + '&Type=WC_VIEW' + '&WebDir=html';
...
function localToUtc(str, format) {
    str = str.trim();
    if (str != null && str != '') {
        var dt = new Date(str);
        return moment.utc(dt).format(format);//'MM/DD/YYYY hh:mm A'
    }
    else
        return '';
}

function UtcToLocal(str, format) {
	if(str){
		str = str.trim();
	    if (str != null && str != '') {
	        var localTime = moment.utc(str).toDate();
	        localTime = moment(localTime).format(format);//'MM/DD/YYYY hh:mm A','M/D/YY hh:mm:ss A'
	        return localTime;
	    }
	    else {
	        return '';
...
function getDate(str, format) {
    str = str.trim();
    if (str != null && str != '') {
        return moment(str).format(format);//'MM/DD/YYYY','M/D/YY'
    }
    else
        return '';
}
...
function getTime(str, format) {
    str = str.trim();
    if (str != null && str != '') {
        return moment(str).format(format);//'hh:mm A'
    }
    else
        return '';
}
